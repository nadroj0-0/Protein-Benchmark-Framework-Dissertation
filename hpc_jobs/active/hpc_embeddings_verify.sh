...
echo
echo "Running embedding generation comparison workflow"
echo "Command:"
echo "bash scripts/reproduction/embeddings_verify.sh"
echo
# Run the workflow but don't immediately exit if it returns an error
bash scripts/reproduction/embeddings_verify.sh
STATUS=$?

...
class WorkflowContractTest(unittest.TestCase):
    ...
    def test_retry_wrapper_is_modality_specific_and_always_cleans_scratch(self) -> None:
        ...
        self.assertIn('REFERENCE_CONTROLS="$WORK_DIR/reference_controls"', workflow)
        self.assertIn("manage_resumable_embedding_state.py\" \\\n  materialize", workflow)
        self.assertIn('--pairs "$CONTROLS"', workflow)
        self.assertIn('--output-cache-root "$REFERENCE_CONTROLS"', workflow)
        self.assertIn('--reference-cache-root "$REFERENCE_CONTROLS"', workflow)
        self.assertIn("control_materialization.json", workflow)
        self.assertIn("--allowed-extra-pairs \"$CONTROLS\"", workflow)
        self.assertIn('"$EMBEDDING_STATE_ROOT/contract.json"', workflow)
        self.assertIn('--baseline-archive "$BASELINE_ARCHIVE"', workflow)
        self.assertIn('--baseline-assembly-report "$BASELINE_ASSEMBLY_REPORT"', workflow)

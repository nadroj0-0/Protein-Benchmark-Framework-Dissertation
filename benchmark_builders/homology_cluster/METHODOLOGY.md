...
Daniel subsequently suggested using UniRef50 as the starting scaffold because every requested
threshold is below 50% identity, and his supplied working script uses sensitivity `4`. The code
therefore preserves the original UniRef90/`7.5` experiment and adds an explicit, separately
namespaced UniRef50/`4` profile. This is treated as a methodological profile change rather than a
...
The explicitly selected UniRef90 or UniRef50 FASTA is the MMseqs2 population. The selected UniProt source controls which GOA
accessions may supply supervised rows and authorize cluster retention.

Production input roles are:

- always: the selected UniRef FASTA, `idmapping_selected`, GOA GAF, GO OBO;
...
authoritative-origin review. The manifest scope must exactly match configured roles. The selected UniRef scaffold,
...
reports per-source primary/secondary counts. Conflicting primary records and duplicate primaries
even with identical sequences fail production rather than depending on incidental input order.
Secondary aliases attached to multiple primaries remain explicitly ambiguous whether those
primaries have identical or conflicting sequences; they are counted, written to
`uniprot_accession_collisions.tsv.gz`, and any qualifying occurrence is excluded rather than
assigned arbitrarily. A GOA
accession absent from the selected source cannot retain a cluster merely because
idmapping links it to a selected-scaffold entry.

Per-source mapping counts preserve distinct stages: selected-UniProt resolution by primary or
secondary accession, mapping to a present selected-scaffold member, and assignment to an MMseqs2 cluster.
...
Both profiles use literal aligned identity with `--alignment-mode 3 --seq-id-mode 0`,
`-c 0.8 --cov-mode 0` for aligned residues divided by the longer sequence, and greedy set-cover
cluster mode 0. The legacy-calibrated profile adds `--cluster-reassign 1`, `-s 7.5`, and
`-e 1e-4`; the UniRef50 launcher uses Daniel's aligned defaults with `-s 4`, default input shuffle
and E-value, and no reassignment. Greedy set cover does not imply
...
- [UniRef50 help](https://www.uniprot.org/help/uniref50)
...
9 and UniRef50 at field 10. Empty fields are preserved. UniProt accession, selected UniRef ID, and MMseqs representative never
...
canonical UniProt sequence, maps uniquely to a present member of the selected UniRef scaffold, and that member has an
MMseqs2 cluster assignment. All selected-scaffold members of retained clusters remain in the retained-member
...
weights are retained selected-scaffold member counts. Deterministic candidate ordering and tie hashes do not
...
- GOA-to-selected-UniProt and selected-UniProt-to-selected-scaffold mapping;
- qualifying annotation-row retention;
- retained selected-scaffold member coverage;
...
2. Treat UniRef90/7.5 and UniRef50/4 as distinct profiles and record which one is used for reported results.
3. Review pilot observations and approve explicit limits for every attrition metric.
4. Confirm provisional memory, scratch, and walltime after the real 30% pilot.
5. Revisit MMseqs2 E-value/reassignment policy after low-identity sensitivity/resource evidence if required.
6. Confirm the established GO relationship and root-retention policies or authorize a revision.
7. Approve a scientifically valid policy before any all-cluster-member supervision.
8. Pin authoritative archived inputs and decide whether a frozen deleted-accession source is needed.
